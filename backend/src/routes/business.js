...
businessRouter.get('/catalog/barcode/:code',async(req,res,next)=>{try{const code=String(req.params.code??'').trim();if(!/^\d{8,14}$/.test(code))return res.status(400).json({error:'Código de barras no válido.'});const response=await fetch(`https://world.openfoodfacts.org/api/v2/product/${code}?fields=code,product_name_es,product_name,categories_tags`,{headers:{'User-Agent':'AbarrotesElPedernal/1.0 (catalog lookup)'}});if(!response.ok)return res.json({encontrado:false,codigo:code});const data=await response.json();const product=data?.product;if(!product)return res.json({encontrado:false,codigo:code});const categoria=Array.isArray(product.categories_tags)&&product.categories_tags.length?String(product.categories_tags[0]).replace(/^[a-z]{2}:/,'').replaceAll('-',' '):'Abarrotes';return res.json({encontrado:true,codigo:code,nombre:String(product.product_name_es||product.product_name||'').trim(),categoria});}catch(error){return next(error);}});
...
businessRouter.post('/products', requireRole('Administrador','Gerente','Cajera'), async(req,res,next)=>{ try { const codigo=String(req.body?.codigo??'').trim()||null,nombre=String(req.body?.nombre??'').trim(),categoria=String(req.body?.categoria??'').trim(),unidadMedida=String(req.body?.unidadMedida??'Pieza').trim();const stock=Number(req.body?.stock??0),stockMinimo=Number(req.body?.stockMinimo??0),costo=Number(req.body?.costo??0),precioVenta=Number(req.body?.precioVenta),tasaIva=Number(req.body?.tasaIva??0);if(!nombre||!categoria||!['Pieza','Kilogramo'].includes(unidadMedida)||![stock,stockMinimo,costo,precioVenta,tasaIva].every(Number.isFinite)||stock<0||stockMinimo<0||costo<0||precioVenta<=0||tasaIva<0||unidadMedida==='Pieza'&&(!Number.isInteger(stock)||!Number.isInteger(stockMinimo)))return res.status(400).json({error:'Nombre, categoría, unidad, existencia y precio de venta válidos son obligatorios.'});if(codigo){const [[existente]]=await pool.execute('SELECT id FROM productos WHERE codigo_barras=? LIMIT 1',[codigo]);if(existente)return res.status(409).json({error:'Ya existe un producto con ese código de barras.'});} const [cats]=await pool.execute('SELECT id FROM categorias WHERE nombre=? LIMIT 1',[categoria]); let categoriaId=cats[0]?.id; if(!categoriaId){const [cat]=await pool.execute('INSERT INTO categorias(nombre) VALUES(?)',[categoria]);categoriaId=cat.insertId;} const [result]=await pool.execute('INSERT INTO productos(categoria_id,codigo_barras,nombre,unidad_medida,stock_actual,stock_minimo,costo,precio_venta,tasa_iva) VALUES(?,?,?,?,?,?,?,?,?)',[categoriaId,codigo,nombre,unidadMedida,stock,stockMinimo,costo,precioVenta,tasaIva]);res.status(201).json({id:result.insertId}); }catch(e){next(e);} });
