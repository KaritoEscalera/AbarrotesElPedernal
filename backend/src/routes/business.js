...
businessRouter.get('/lots/alerts',requireRole('Administrador','Gerente'),async(_req,res,next)=>{try{const [rows]=await pool.query(`SELECT l.id,l.producto_id AS productoId,p.nombre,l.lote,l.fecha_caducidad AS fechaCaducidad,l.cantidad_disponible AS cantidad,DATEDIFF(l.fecha_caducidad,CURRENT_DATE) AS dias FROM producto_lotes l JOIN productos p ON p.id=l.producto_id WHERE p.activo=TRUE AND l.cantidad_disponible>0 AND l.fecha_caducidad IS NOT NULL AND l.fecha_caducidad<=DATE_ADD(CURRENT_DATE,INTERVAL 30 DAY) ORDER BY l.fecha_caducidad`);res.json(rows);}catch(e){next(e);}});
businessRouter.post('/lots/:id/provider-exchange',requireRole('Administrador','Gerente'),async(req,res,next)=>{const connection=await pool.getConnection();try{const loteNuevo=String(req.body?.loteNuevo??'').trim(),fechaCaducidad=String(req.body?.fechaCaducidad??'').trim(),nota=String(req.body?.nota??'').trim();if((loteNuevo&&!fechaCaducidad)||(!loteNuevo&&fechaCaducidad)||(fechaCaducidad&&(!/^\d{4}-\d{2}-\d{2}$/.test(fechaCaducidad)||Number.isNaN(new Date(`${fechaCaducidad}T00:00:00`).getTime()))))return res.status(400).json({error:'Captura lote y caducidad juntos, o deja ambos vacíos para marcar el cambio como atendido.'});await connection.beginTransaction();const [[lot]]=await connection.execute(`SELECT l.*,p.stock_actual,p.nombre,(SELECT pr.empresa FROM proveedor_productos pp JOIN proveedores pr ON pr.id=pp.proveedor_id WHERE pp.producto_id=l.producto_id AND pp.es_principal=TRUE LIMIT 1) AS proveedor FROM producto_lotes l JOIN productos p ON p.id=l.producto_id WHERE l.id=? FOR UPDATE`,[req.params.id]);if(!lot||Number(lot.cantidad_disponible)<=0){await connection.rollback();return res.status(409).json({error:'El lote ya fue atendido o no tiene existencias.'});}const cantidad=Number(lot.cantidad_disponible),loteFinal=loteNuevo||lot.lote,motivo=`Cambio con proveedor${lot.proveedor?` ${lot.proveedor}`:''}: lote ${lot.lote}${loteNuevo?` por ${loteNuevo}`:' marcado como atendido'}${nota?` · ${nota}`:''}`;if(!loteNuevo)await connection.execute('UPDATE producto_lotes SET fecha_caducidad=NULL WHERE id=?',[lot.id]);else if(loteNuevo===lot.lote)await connection.execute('UPDATE producto_lotes SET fecha_caducidad=? WHERE id=?',[fechaCaducidad,lot.id]);else{await connection.execute('UPDATE producto_lotes SET cantidad_disponible=0 WHERE id=?',[lot.id]);await connection.execute(`INSERT INTO producto_lotes(producto_id,lote,fecha_caducidad,cantidad_inicial,cantidad_disponible,costo_unitario) VALUES(?,?,?,?,?,?) ON DUPLICATE KEY UPDATE cantidad_inicial=cantidad_inicial+VALUES(cantidad_inicial),cantidad_disponible=cantidad_disponible+VALUES(cantidad_disponible),fecha_caducidad=VALUES(fecha_caducidad),costo_unitario=VALUES(costo_unitario)`,[lot.producto_id,loteNuevo,fechaCaducidad,cantidad,cantidad,lot.costo_unitario]);}await connection.execute(`INSERT INTO movimientos_inventario(producto_id,usuario_id,tipo,cantidad,stock_anterior,stock_nuevo,costo_unitario,referencia_tipo,referencia_id,motivo) VALUES(?,?,'AJUSTE',?,?,?,?, 'CAMBIO_PROVEEDOR',?,?)`,[lot.producto_id,req.user.sub,cantidad,lot.stock_actual,lot.stock_actual,lot.costo_unitario,lot.id,motivo]);await connection.execute(`INSERT INTO bitacora(usuario_id,modulo,accion,descripcion,entidad,entidad_id) VALUES(?,'Inventario','CAMBIO_PROVEEDOR',?,'LOTE',?)`,[req.user.sub,`${lot.nombre}: ${motivo}`,lot.id]);await connection.commit();return res.json({stock:Number(lot.stock_actual),cantidad,loteNuevo:loteFinal,fechaCaducidad:fechaCaducidad||null});}catch(error){await connection.rollback();return next(error);}finally{connection.release();}});
...
businessRouter.post('/products', requireRole('Administrador','Gerente'), async(req,res,next)=>{
  ...
  try {
    ...
    const stock=Number(req.body?.stock??0),stockMinimo=Number(req.body?.stockMinimo??0),costo=req.body?.costo===null||req.body?.costo===''||req.body?.costo===undefined?null:Number(req.body.costo),precioVenta=Number(req.body?.precioVenta),tasaIva=Number(req.body?.tasaIva??0),proveedorId=Number(req.body?.proveedorId)||null,numeroProveedor=String(req.body?.numeroProveedor??'').trim()||null;
    if(!nombre||!categoria||!['Pieza','Kilogramo'].includes(unidadMedida)||![stock,stockMinimo,precioVenta,tasaIva].every(Number.isFinite)||(costo!==null&&(!Number.isFinite(costo)||costo<0))||stock<0||stockMinimo<0||precioVenta<=0||tasaIva<0||unidadMedida==='Pieza'&&(!Number.isInteger(stock)||!Number.isInteger(stockMinimo)))return res.status(400).json({error:'Nombre, categoría, unidad, existencia y precio de venta válidos son obligatorios.'});
...
businessRouter.get('/providers', async(_req,res,next)=>{try{const [rows]=await pool.query(`SELECT id,empresa,contacto AS nombre,telefono,correo,producto_principal AS productoPrincipal,dia_entrega AS diaEntrega,DATE_FORMAT(ultima_compra,'%Y-%m-%d') AS ultimaCompra,DATE_FORMAT(proxima_entrega,'%Y-%m-%d') AS proximaEntrega,CASE estado_pedido WHEN 'SIN_PEDIDO' THEN 'Sin pedido' WHEN 'PENDIENTE' THEN 'Pendiente' WHEN 'RECIBIDO' THEN 'Recibido' ELSE 'Atrasado' END AS estadoPedido,saldo_pendiente AS saldoPendiente,estado FROM proveedores WHERE estado<>'ARCHIVADO' ORDER BY empresa`);res.json(rows);}catch(e){next(e);}});
businessRouter.post('/providers',requireRole('Administrador','Gerente'),async(req,res,next)=>{try{const p=req.body,empresa=String(p.empresa??'').trim(),nombre=String(p.nombre??'').trim(),telefono=String(p.telefono??'').trim(),correo=String(p.correo??'').trim().toLowerCase()||null,producto=String(p.productoPrincipal??'').trim(),dia=String(p.diaEntrega??'').trim(),saldo=Number(p.saldoPendiente??0),estadoPedido=String(p.estadoPedido??'Sin pedido').toUpperCase().replaceAll(' ','_');if(!empresa||!nombre||!producto||!dia||telefono&&!/^\d{10}$/.test(telefono)||!Number.isFinite(saldo)||saldo<0||!['SIN_PEDIDO','PENDIENTE','RECIBIDO','ATRASADO'].includes(estadoPedido))return res.status(400).json({error:'Los datos del proveedor no son válidos.'});const [r]=await pool.execute(`INSERT INTO proveedores(empresa,contacto,telefono,correo,producto_principal,dia_entrega,ultima_compra,proxima_entrega,estado_pedido,saldo_pendiente,estado) VALUES(?,?,?,?,?,?,?,?,?,?,'ACTIVO')`,[empresa,nombre,telefono,correo,producto,dia,p.ultimaCompra||null,p.proximaEntrega||null,estadoPedido,saldo]);await pool.execute(`INSERT INTO bitacora(usuario_id,modulo,accion,descripcion,entidad,entidad_id) VALUES(?,'Proveedores','CREAR',?,'PROVEEDOR',?)`,[req.user.sub,empresa,r.insertId]);res.status(201).json({id:r.insertId});}catch(e){if(e?.code==='ER_DUP_ENTRY')return res.status(409).json({error:'Ya existe un proveedor con esa empresa o correo.'});next(e);}});
businessRouter.put('/providers/:id',requireRole('Administrador','Gerente'),async(req,res,next)=>{try{const p=req.body,empresa=String(p.empresa??'').trim(),nombre=String(p.nombre??'').trim(),telefono=String(p.telefono??'').trim(),correo=String(p.correo??'').trim().toLowerCase()||null,producto=String(p.productoPrincipal??'').trim(),dia=String(p.diaEntrega??'').trim(),saldo=Number(p.saldoPendiente??0),estadoPedido=String(p.estadoPedido??'Sin pedido').toUpperCase().replaceAll(' ','_'),estado=String(p.estado??'Activo').toUpperCase();if(!empresa||!nombre||!producto||!dia||telefono&&!/^\d{10}$/.test(telefono)||!Number.isFinite(saldo)||saldo<0||!['SIN_PEDIDO','PENDIENTE','RECIBIDO','ATRASADO'].includes(estadoPedido)||!['ACTIVO','INACTIVO'].includes(estado))return res.status(400).json({error:'Los datos del proveedor no son válidos.'});const [result]=await pool.execute(`UPDATE proveedores SET empresa=?,contacto=?,telefono=?,correo=?,producto_principal=?,dia_entrega=?,ultima_compra=?,proxima_entrega=?,estado_pedido=?,saldo_pendiente=?,estado=? WHERE id=? AND estado<>'ARCHIVADO'`,[empresa,nombre,telefono,correo,producto,dia,p.ultimaCompra||null,p.proximaEntrega||null,estadoPedido,saldo,estado,req.params.id]);if(!result.affectedRows)return res.status(404).json({error:'Proveedor no encontrado.'});await pool.execute(`INSERT INTO bitacora(usuario_id,modulo,accion,descripcion,entidad,entidad_id) VALUES(?,'Proveedores','ACTUALIZAR',?,'PROVEEDOR',?)`,[req.user.sub,empresa,req.params.id]);res.status(204).end();}catch(e){if(e?.code==='ER_DUP_ENTRY')return res.status(409).json({error:'Ya existe un proveedor con esa empresa o correo.'});next(e);}});
    ...
    const [salesResult, productsResult, creditsResult, cashResult, remindersResult] = await Promise.all([
      ...
      pool.query(
        `SELECT tipo,titulo,detalle,nivel,ruta FROM (
           ...
           FROM producto_lotes l JOIN productos p ON p.id=l.producto_id WHERE p.activo=TRUE AND l.cantidad_disponible>0 AND l.fecha_caducidad<=DATE_ADD(CURRENT_DATE,INTERVAL 30 DAY)
           UNION ALL
  ...
  try {
    ...
    const [totalsResult, movementsResult] = await Promise.all([
      pool.execute(
        `SELECT
          ...
          COALESCE(SUM(CASE WHEN categoria='COMPRA' AND metodo='TRANSFERENCIA' THEN monto ELSE 0 END),0) AS comprasTransferencia
    ...
    return res.json({
      ...
      totals: {
        ...
        comprasCaja: Number(totals.comprasCaja),
        comprasEfectivo: Number(totals.comprasEfectivo),
        comprasTarjeta: Number(totals.comprasTarjeta),
        comprasTerminal: Number(totals.comprasTerminal),
        comprasTransferencia: Number(totals.comprasTransferencia),
      },
      movements,
    });
  } catch (error) { return next(error); }
  ...
  try {
    ...
    const [[totals]] = await connection.execute(
      `SELECT COALESCE(SUM(CASE WHEN tipo='INGRESO' AND metodo='EFECTIVO' THEN monto ELSE 0 END),0) ingresos,
              ...
              COALESCE(SUM(CASE WHEN categoria='COMPRA' AND metodo='TRANSFERENCIA' THEN monto ELSE 0 END),0) comprasTransferencia
       FROM movimientos_caja WHERE sesion_caja_id=?`,
      [session.id],
    );
    ...
    await connection.commit();
    return res.json({ sesionId: session.id, fondoInicial: Number(session.fondo_inicial), efectivoEsperado: esperado, efectivoContado, diferencia, ventasEfectivo:Number(totals.ventasEfectivo),ventasTarjeta:Number(totals.ventasTarjeta),ventasTransferencia:Number(totals.ventasTransferencia),ventasFiado:Number(totals.ventasFiado),comprasCaja:Number(totals.comprasCaja),comprasEfectivo:Number(totals.comprasEfectivo),comprasTarjeta:Number(totals.comprasTarjeta),comprasTerminal:Number(totals.comprasTerminal),comprasTransferencia:Number(totals.comprasTransferencia),fechaCierre:new Date().toISOString() });
...
businessRouter.post('/sales', async (req, res, next) => {
  ...
  try {
    ...
    for (const [productoId, cantidad] of normalized) {
      ...
      if (product.costo === null) { await connection.rollback(); return res.status(409).json({ error: `${product.nombre} tiene costo pendiente. Registra una compra o entrada con costo real antes de venderlo.` }); }
  ...
  try {
    const [rows] = await pool.query(
      `SELECT c.id,c.folio,c.fecha_compra AS fecha,c.estado,c.metodo_pago AS metodoPago,c.total,c.saldo_pendiente AS saldoPendiente,
              COALESCE((SELECT GROUP_CONCAT(CONCAT(cp.metodo,' ',cp.origen,' $',FORMAT(cp.monto,2)) ORDER BY cp.id SEPARATOR ' · ') FROM compra_pagos cp WHERE cp.compra_id=c.id),IF(c.metodo_pago='CREDITO','CRÉDITO','Sin desglose')) AS pagoDetalle,
...
businessRouter.post('/purchases', requireRole('Administrador','Gerente'), async (req, res, next) => {
  ...
  try {
    ...
    let taxes = 0;
    for (const item of items) {
      const productoId = Number(item?.productoId); const cantidad = Number(item?.cantidad); const costo = item?.costoUnitario===null||item?.costoUnitario===''||item?.costoUnitario===undefined?NaN:Number(item.costoUnitario);
    ...
    for(const pago of pagosCompra)await connection.execute(`INSERT INTO compra_pagos(compra_id,origen,metodo,monto) VALUES(?,?,?,?)`,[purchase.insertId,pago.origen,pago.metodo,pago.monto]);
    for (const detail of details) {
      await connection.execute(
