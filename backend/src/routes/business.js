...
businessRouter.get('/customer-balances',async(_req,res,next)=>{try{const [rows]=await pool.query(`SELECT s.id,s.cliente_id AS clienteId,c.nombre AS cliente,c.telefono,s.monto,s.estado,s.creado_en AS fecha FROM saldos_clientes s JOIN clientes c ON c.id=s.cliente_id WHERE s.estado='PENDIENTE' ORDER BY s.creado_en DESC`);return res.json(rows);}catch(error){return next(error);}});
...
businessRouter.post('/sales', async (req, res, next) => {
  ...
  try {
    ...
    if(montoFiado>0){const [[client]]=await connection.execute(`SELECT limite_credito,(SELECT COALESCE(SUM(saldo_pendiente),0) FROM fiados WHERE cliente_id=clientes.id AND estado NOT IN('LIQUIDADO','CANCELADO')) AS adeudo FROM clientes WHERE id=? AND activo=TRUE FOR UPDATE`,[clienteId]);if(!client){await connection.rollback();return res.status(400).json({error:'Cliente no válido.'});}const disponible=Number(client.limite_credito)-Number(client.adeudo);if(Number(client.limite_credito)>0&&montoFiado>disponible){await connection.rollback();return res.status(409).json({error:`El fiado supera el crédito disponible de $${Math.max(0,disponible).toFixed(2)}.`});}}
