...
interface Partida { productoId: number | null; cantidad: number | null; costoUnitario: number | null; }
...
export class ComprasPage implements OnInit {
  ...
  proveedores: Proveedor[] = []; productos: Producto[] = []; compras: Compra[] = [];
  ...
  private nuevaPartida(): Partida { return { productoId: null, cantidad: 1, costoUnitario: null }; }
  private async cargar(): Promise<void> {
    try {
      const [providers, products, purchases] = await Promise.all([this.api.get<Proveedor[]>('providers'), this.api.get<Producto[]>('products'), this.api.get<Compra[]>('purchases')]);
