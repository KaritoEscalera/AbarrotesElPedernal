...
interface SaludSistema { api:{estado:string;uptimeSegundos:number};database:{estado:string;latenciaMs:number;conexiones:number};backup:{estado:string;fecha:string|null;error:string|null};disk:{libreBytes:number;totalBytes:number};offsiteBackup:{configurado:boolean};ambiente:string;version:string; }
...
export class RespaldosPage implements OnInit {
  ...
  respaldos:Respaldo[]=[];salud:SaludSistema|null=null;archivo:File|null=null;confirmacion='';mensaje='';error='';procesando=false;
  async ngOnInit():Promise<void>{await Promise.all([this.cargar(),this.cargarSalud()]);}
  ...
  porcentajeDisco():number{return this.salud?.disk.totalBytes?Math.round((1-this.salud.disk.libreBytes/this.salud.disk.totalBytes)*100):0;}
  antiguedadRespaldo():string{if(!this.salud?.backup.fecha)return'Sin respaldo';const horas=Math.floor((Date.now()-new Date(this.salud.backup.fecha).getTime())/3600000);return horas<1?'Hace menos de 1 hora':`Hace ${horas} h`;}
  ...
  private async cargarSalud():Promise<void>{try{this.salud=await this.api.get<SaludSistema>('system/health');}catch{this.salud=null;}}
