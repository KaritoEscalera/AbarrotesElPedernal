...
export class InventarioPage implements OnInit {
  ...
  async eliminarProducto(producto:ProductoInventario):Promise<void>{if(!confirm(`¿Eliminar ${producto.nombre}? Se quitará de Caja, pero su historial de ventas se conservará.`))return;try{await this.api.delete(`products/${producto.id}`);await this.cargarProductos();this.mensajeEntrada='Producto eliminado del catálogo; su historial se conservó.';}catch{this.mensajeEntrada='No fue posible eliminar el producto.';}}
